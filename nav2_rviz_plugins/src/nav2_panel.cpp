...
Nav2Panel::Nav2Panel(QWidget* parent)
    ...
    navigation_action_client_ =
        rclcpp_action::create_client<nav2_msgs::action::NavigateToPose>(client_node_, "navigate_to_pose");
...
void Nav2Panel::onInitialize() {
    ...
    navigation_goal_status_sub_ = node->create_subscription<action_msgs::msg::GoalStatusArray>(
        ...
        [this](const action_msgs::msg::GoalStatusArray::SharedPtr msg) {
            navigation_goal_status_indicator_->setText(getGoalStatusLabel(msg->status_list.back().status));
...
void Nav2Panel::onPause() {
    QFuture<void> futureNav = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::pause, &client_nav_, std::placeholders::_1),
        server_timeout_);
    QFuture<void> futureLoc = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::pause, &client_loc_, std::placeholders::_1),
        server_timeout_);
}

void Nav2Panel::onResume() {
    QFuture<void> futureNav = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::resume, &client_nav_, std::placeholders::_1),
        server_timeout_);
    QFuture<void> futureLoc = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::resume, &client_loc_, std::placeholders::_1),
        server_timeout_);
}

void Nav2Panel::onStartup() {
    QFuture<void> futureNav = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::startup, &client_nav_, std::placeholders::_1),
        server_timeout_);
    QFuture<void> futureLoc = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::startup, &client_loc_, std::placeholders::_1),
        server_timeout_);
}

void Nav2Panel::onShutdown() {
    QFuture<void> futureNav = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::reset, &client_nav_, std::placeholders::_1),
        server_timeout_);
    QFuture<void> futureLoc = QtConcurrent::run(
        std::bind(&nav2_lifecycle_manager::LifecycleManagerClient::reset, &client_loc_, std::placeholders::_1),
        server_timeout_);
    timer_.stop();
}
...
inline QString Nav2Panel::getGoalStatusLabel(int8_t status) {
    ...
    return QString(
        std::string("<table><tr><td width=100><b>Feedback:</b></td><td>" + status_str + "</td></tr></table>").c_str());
}
...
inline QString Nav2Panel::getNavThroughPosesFeedbackLabel(nav2_msgs::action::NavigateThroughPoses::Feedback msg) {
    return QString(std::string("<table><tr><td width=150>Poses remaining:</td><td>" +
                               std::to_string(msg.number_of_poses_remaining) + "</td></tr>" + toLabel(msg) + "</table>")
                       .c_str());
}
