...
public:
    ...
    void initialize(const bool &allow_unknown, int &max_iterations, const int &max_on_approach_iterations,
                    const float &lookup_table_size, const unsigned int &dim_3_size);
...
protected:
     ...
     * return nullptr
     */
    NodePtr tryAnalyticExpansion(const NodePtr &current_node, const NodeGetter &getter, int &iterations,
                                 int &best_cost);
    ...
    bool _traverse_unknown; // Whether or not to traverse in unknown space
    int _max_iterations;
