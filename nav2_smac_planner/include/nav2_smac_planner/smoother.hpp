...
public:
     ...
     */
    void initialize(const double& min_turning_radius) { min_turning_rad_ = min_turning_radius; }

    /**
     * @brief Smoother method
     * @param path Reference to path
     * @param costmap Pointer to minimal costmap
     * @param max_time 最大计算时间，超时了会提前停止
    ...
    bool smooth(nav_msgs::msg::Path& path, const nav2_costmap_2d::Costmap2D* costmap, const double& max_time,
        ...
        while (change >= tolerance_) {
            ...
            // 确保平滑函数收敛，如果超过迭代次数限制，那么就取最后一次平滑的结果，不过返回的false没有用到
            if (its >= max_its_) {
            ...
            for (unsigned int i = 2; i != path_size - 2; i++) {
                ...
                // 验证更新是可接受的，只check cost(如果costmap的指针提供了)
                float cost = 0.0;
                if (costmap) {
                    costmap->worldToMap(getFieldByDim(new_path.poses[i], 0), getFieldByDim(new_path.poses[i], 1), mx,
                                        my);
