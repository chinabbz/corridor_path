...
void AStarAlgorithm<NodeT>::initialize(const bool& allow_unknown, int& max_iterations,
                                       const int& max_on_approach_iterations, const float& lookup_table_size,
...
template <>
void AStarAlgorithm<Node2D>::initialize(const bool& allow_unknown, int& max_iterations,
                                        const int& max_on_approach_iterations, const float& /*lookup_table_size*/,
                                        const unsigned int& dim_3_size) {
    _traverse_unknown = allow_unknown;
    _max_iterations = max_iterations;
...
typename AStarAlgorithm<NodeT>::AnalyticExpansionNodes AStarAlgorithm<NodeT>::getAnalyticPath(
    const NodePtr& node, const NodeGetter& node_getter) {
    static ompl::base::ScopedState<> from(node->motion_table.state_space), to(node->motion_table.state_space),
    ...
    for (float i = 1; i < num_intervals; i++) {
        ...
        index = NodeT::getIndex(static_cast<unsigned int>(reals[0]), static_cast<unsigned int>(reals[1]),
...
typename AStarAlgorithm<NodeT>::NodePtr AStarAlgorithm<NodeT>::setAnalyticPath(
    const NodePtr& node, const AnalyticExpansionNodes& expanded_nodes) {
