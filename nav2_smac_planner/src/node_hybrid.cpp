...
void HybridMotionTable::initDubin(unsigned int &size_x_in, unsigned int & /*size_y_in*/,
    ...
    if (num_angle_quantization_in == num_angle_quantization &&
        min_turning_radius == search_info.minimum_turning_radius && motion_model == MotionModel::DUBIN) {
        return;
    }
...
void HybridMotionTable::initReedsShepp(unsigned int &size_x_in, unsigned int & /*size_y_in*/,
                                       unsigned int &num_angle_quantization_in, SearchInfo &search_info) {
    size_x = size_x_in;
    change_penalty = search_info.change_penalty;
    non_straight_penalty = search_info.non_straight_penalty;
    ...
    // if nothing changed, no need to re-compute primitives
    if (num_angle_quantization_in == num_angle_quantization &&
        min_turning_radius == search_info.minimum_turning_radius && motion_model == MotionModel::REEDS_SHEPP) {
        return;
    }
...
MotionPoses HybridMotionTable::getProjections(const NodeHybrid *node) {
    ...
    for (unsigned int i = 0; i != projections.size(); i++) {
        ...
        projection_list.emplace_back(delta_xs[i][node_heading] + node->pose.x, delta_ys[i][node_heading] + node->pose.y,
                                     new_heading);
    }
...
bool NodeHybrid::isNodeValid(const bool &traverse_unknown, GridCollisionChecker *collision_checker) {
    if (collision_checker->inCollision(this->pose.x, this->pose.y, this->pose.theta * motion_table.bin_size,
                                       traverse_unknown)) {
        return false;
    }
...
float NodeHybrid::getTraversalCost(const NodePtr &child) {
    ...
    if (child->getMotionPrimitiveIndex() == 0 || child->getMotionPrimitiveIndex() == 3) {
        // 新的动作是直线，则不增加惩罚（奇怪的是好像也没有对倒车进行惩罚）
        travel_cost = travel_cost_raw;
    } else {
        if (getMotionPrimitiveIndex() == child->getMotionPrimitiveIndex()) {
            // 转向，但是和上一次是同方向的转向: encourages to commit to turning if starting it
...
void NodeHybrid::initMotionModel(const MotionModel &motion_model, unsigned int &size_x, unsigned int &size_y,
...
void NodeHybrid::resetObstacleHeuristic(nav2_costmap_2d::Costmap2D *costmap, const unsigned int &goal_x,
    ...
    // 2倍降采样costmap来计算来计算稀疏的障碍启发式。
    // 这会大大加速planner来少搜索75%的cells，并且即使是在适度的平滑之后也不会对路径产生可检测的侵蚀
    // 误差不会超过0.05*归一化cost。因为这只是先验搜索，一般来说不会有loss
    ...
    obstacle_heuristic_queue.emplace(ceil(goal_y / 2.0) * sampled_costmap->getSizeInCellsX() + ceil(goal_x / 2.0));
}

float NodeHybrid::getObstacleHeuristic(const Coordinates &node_coords, const Coordinates &goal_coords) {
    // 如果已经被扩展过，就直接返回cost
    unsigned int size_x = sampled_costmap->getSizeInCellsX();
    // Divided by 2 due to downsampled costmap.
    const unsigned int start_index = ceil(node_coords.y / 2.0) * size_x + ceil(node_coords.x / 2.0); //当前位置
    ...
    while (!obstacle_heuristic_queue.empty()) {
        ...
        for (unsigned int i = 0; i != neighborhood.size(); i++) {
            ...
            travel_cost =
                ((i <= 3) ? 1.0 : sqrt_2) + (motion_table.cost_penalty * cost / 252.0); //距离cost+成本地图cost
            ...
            if (new_idx > 0 && new_idx < size_x * size_y && cost < INSCRIBED) {
                ...
                // 如果当前邻居节点的障碍启发式函数未分配或者大于经过当前节点到达的cost
...
float NodeHybrid::getDistanceHeuristic(const Coordinates &node_coords, const Coordinates &goal_coords,
    ...
    // 利用旋转平移来使终点的坐标为(0,0,0)，因为dist_heuristic_lookup_table是预先计算的，这个时候不知道终点在哪，就以(0,0,0)为终点
    ...
    Coordinates node_coords_relative(round(dx * cos_th - dy * sin_th), round(dx * sin_th + dy * cos_th),
                                     round(dtheta_bin));
...
void NodeHybrid::getNeighbors(
    ...
    // 利用运动模型来检索邻居节点，DUBIN曲线只有3种，R-S曲线会有6种
    for (unsigned int i = 0; i != motion_projections.size(); i++) {
        ...
        if (NeighborGetter(index, neighbor) && !neighbor->wasVisited()) {
            ...
            neighbor->setPose(
                Coordinates(motion_projections[i]._x, motion_projections[i]._y, motion_projections[i]._theta));
